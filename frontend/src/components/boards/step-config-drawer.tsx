...
export function StepConfigDrawer({ boardId, step, allSteps, onClose }: StepConfigDrawerProps) {
    ...
    const [triggerType, setTriggerType] = useState(step.trigger_type || 'on_entry')
        ...
        <>
            ...
            <div className="fixed right-0 top-0 bottom-0 z-50 w-[520px] max-w-[95vw] bg-card border-l border-border shadow-2xl flex flex-col animate-in slide-in-from-right-full duration-200">
                ...
                <div className="flex-1 overflow-y-auto min-h-0 px-6 py-5 space-y-6">
                    ...
                    {hasCategory && step.linked_category && (
                        <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-primary/5 border border-primary/10">
                            ...
                            <div className="text-xs">
                                ...
                                <span className="text-muted-foreground"> — inherits Skills, Knowledge & System Prompt</span>
                            </div>
                        </div>
                    )}
                    ...
                    {/* ─── Webhook Config (when trigger = webhook) ── */}
                    {triggerType === 'webhook' && (
                    ...
                    {/* ─── Schedule Config (when trigger = schedule) ── */}
...
function SchemaFieldRow({ field, showRequired, onChange, onRemove }: SchemaFieldRowProps) {
    ...
    // Render the appropriate default value input based on field type
    ...
    return (
        <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2 group">
                ...
                <select
                    ...
                    onChange={(e) => {
                        ...
                        // Clear default_value when switching types so it doesn't carry invalid data
            ...
            {/* Dropdown options editor (shown BEFORE default so user defines options first) */}
            {field.type === 'dropdown' && (
