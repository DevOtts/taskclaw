    ...
    return (
        <div className="w-72 flex flex-col flex-shrink-0">
            ...
            <div
                ref={setNodeRef}
                className={cn(
                    'flex flex-col gap-2.5 flex-1 min-h-[100px] p-1 rounded-lg transition-colors',
            ...
            >
                ...
                {status !== 'Done' && status !== 'AI Running' && status !== 'In Review' && (
                    <button
                        onClick={onAddTask}
                        className="border border-dashed border-border p-2.5 rounded-lg text-muted-foreground text-xs font-medium hover:bg-accent hover:text-accent-foreground transition-all flex items-center justify-center gap-2"
                    >
