...
export async function initializeI18nClient(
  ...
  await i18next
    .use(
      resourcesToBackend(async (language, namespace, callback) => {
        if (resources && resources[namespace]) {
          return callback(null, resources[namespace]);
        }
    ...
    .init(
      {
        ...settings,
        resources: resources ? { [settings.lng as string]: resources } : undefined,
        detection: {
  ...
  if (!resources && (loadedLanguages.length === 0 || loadedNamespaces.length === 0)) {
    iteration++;
