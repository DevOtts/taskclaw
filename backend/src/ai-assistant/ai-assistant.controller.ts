...
export class AiAssistantController {
    ...
    @Post('chat')
    async chat(@Body() body: { message: string; history: any[]; conversationId?: string; systemPromptKey?: string }, @Req() req) {
        // Current user context is available in req.user if needed
        const user = req.user;

        return this.aiAssistantService.chat(body.message, body.history, user, body.conversationId, body.systemPromptKey);
    }
    ...
    async updateConversation(@Req() req, @Param('id') id: string, @Body() body: { title?: string; isPublic?: boolean }) {
        if (!req.user?.id) throw new Error('Unauthorized');

        if (body.title !== undefined) {
            return this.aiAssistantService.updateConversationTitle(id, req.user.id, body.title);
        }

        if (body.isPublic !== undefined) {
            return this.aiAssistantService.updateConversationVisibility(id, req.user.id, body.isPublic);
        }
    ...
    async generateEmbeddings(
        ...
        try {
            if (entity_type === 'projects') {
                ...
                const { data: projects, error } = await query.not('description', 'is', null);
                ...
                for (let i = 0; i < projects.length; i += batch_size) {
                    ...
                    for (const project of batch) {
                        ...
                        try {
                            const embedding = await this.embeddingService.generateEmbedding(project.description);
                            await supabase
                                .from('projects')
                ...
                const { data: messages, error } = await query.not('content', 'is', null);
