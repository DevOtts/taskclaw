...
export class AiAssistantService {
    ...
    constructor(
        ...
        const modelName = this.configService.get<string>('AI_MODEL') || 'openai/gpt-4o-mini';
    ...
    async chat(message: string, history: any[] = [], user?: { id: string; email?: string }, conversationId?: string, systemPromptKey: string = 'default') {
        try {
            if (!this.configService.get<string>('OPENROUTER_API_KEY')) {
                return {
                    response: 'AI Assistant is not configured. Please set OPENROUTER_API_KEY.',
                };
            }
            ...
            let baseSystemPrompt = PROMPTS[systemPromptKey] || PROMPTS['default'];
...
DO NOT use current_setting('my.user_id') or similar PostgreSQL session variables as they are not available. Use the explicit UUID provided above.`;
            ...
            const semanticSearchTool = new DynamicStructuredTool({
                ...
                description: "Search for projects, users, or AI conversation messages using natural language semantic search. Use this when the user asks for conceptual matches (e.g., 'find projects about authentication', 'search for conversations about billing').",
                schema: z.object({
                    query: z.string().describe("The natural language search query"),
                    entity_type: z.enum(['projects', 'users', 'messages']).describe("The type of entity to search: 'projects', 'users', or 'messages'"),
                    conversation_id: z.string().optional().describe("For 'messages' search only: filter by specific conversation ID"),
                ...
                func: async ({ query, entity_type, conversation_id, limit = 10 }) => {
                    ...
                    try {
                        ...
                        if (entity_type === 'projects') {
                            const { data, error } = await client.rpc('search_projects_vector', {
                            ...
                            const { data, error } = await client.rpc('search_messages_vector', {
                                query_embedding: JSON.stringify(queryEmbedding),
    ...
    async updateConversationVisibility(conversationId: string, userId: string, isPublic: boolean) {
        const client = this.supabaseService.getAdminClient();
        const { error } = await client
            .from('ai_conversations')
