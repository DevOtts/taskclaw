...
export class AccountsService {
    ...
    async createAccount(userId: string, name: string, accessToken?: string) {
        ...
        try {
            ...
            if (!settings.allow_multiple_projects) {
                ...
                await this.projectsService.createProject(account.id, 'Default Project', userId, accessToken || '');
            }
        } catch (error) {
    ...
    async updateAccount(accountId: string, name: string, userId: string, accessToken?: string) {
        const supabase = this.supabaseService.getClient(accessToken);
