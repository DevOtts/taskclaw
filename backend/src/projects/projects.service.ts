...
export class ProjectsService {
    ...
    async getAccountProjects(accountId: string, userId: string, accessToken?: string) {
    ...
    async createProject(accountId: string, name: string, description: string | undefined, userId: string, accessToken?: string) {
        const supabase = this.supabaseService.getClient(accessToken);
        ...
        if (description && this.embeddingService.isConfigured()) {
            try {
                ...
                this.logger.warn(`Failed to generate embedding for project description: ${error.message}`);
    ...
    async getProjectDetails(projectId: string, userId: string, accessToken?: string) {
        const supabase = this.supabaseService.getClient(accessToken);

        // Verify access via helper (checks project existence + account membership)
        await this.accessControlHelper.verifyProjectAccess(supabase, projectId, userId);
    ...
    async updateProject(projectId: string, name: string, description: string | undefined, userId: string, accessToken?: string) {
        const supabase = this.supabaseService.getClient(accessToken);
    ...
    async deleteProject(projectId: string, userId: string, accessToken?: string) {
        ...
        const { accountId } = await this.accessControlHelper.verifyProjectAccess(supabase, projectId, userId);
    ...
    async findAllProjects(page: number = 1, limit: number = 10, search?: string) {
        ...
        if (search && this.embeddingService.isConfigured()) {
            try {
                const queryEmbedding = await this.embeddingService.generateEmbedding(search);
                
                // Use vector search RPC function
                const { data: vectorResults, error: vectorError } = await supabase.rpc('search_projects_vector', {
                ...
                if (!vectorError && vectorResults && vectorResults.length > 0) {
                    ...
                    this.logger.debug(`Vector search returned ${vectorResults.length} results for query: ${search}`);
                ...
                this.logger.warn(`Vector search failed, falling back to ILIKE: ${error.message}`);
