...
export class ProjectsController {
    ...
    @Put(':id')
    updateProject(@Param('id') id: string, @Body() body: { name: string; description?: string }, @Request() req) {
        const token = req.headers.authorization?.split(' ')[1];
        return this.projectsService.updateProject(id, body.name, body.description, req.user.id, token);
    }
...
export class AccountProjectsController {
    ...
    getAccountProjects(@Param('accountId') accountId: string, @Request() req) {
        const token = req.headers.authorization?.split(' ')[1];
        return this.projectsService.getAccountProjects(accountId, req.user.id, token);
    }
    ...
    createProject(
        ...
        const token = req.headers.authorization?.split(' ')[1];
        return this.projectsService.createProject(accountId, body.name, body.description, req.user.id, token);
    }
}
