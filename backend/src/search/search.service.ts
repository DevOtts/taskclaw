...
export class SearchService {
    ...
    private async hybridSearch(query: string) {
        ...
        try {
            ...
            if (accountsRes.error) throw new InternalServerErrorException(accountsRes.error.message);
    ...
    private async ilikeSearch(query: string) {
        ...
        if (accountsRes.error) throw new InternalServerErrorException(accountsRes.error.message);
        if (projectsRes.error) throw new InternalServerErrorException(projectsRes.error.message);
     ...
     */
    private mergeResults(vectorResults: SearchResult[], ilikeResults: SearchResult[]): SearchResult[] {
