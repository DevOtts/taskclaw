...
describe('AnthropicAdapter', () => {
  ...
  it('validateConfig passes with api_key', () => {
    expect(() =>
      adapter.validateConfig({ api_key: 'sk-ant-test' }),
    ).not.toThrow();
  });

  (SKIP ? it.skip : it)(
    'healthCheck returns healthy with valid key',
    async () => {
  ...
  (SKIP ? it.skip : it)(
    'healthCheck returns unhealthy with invalid key',
    async () => {
  ...
  (SKIP ? it.skip : it)(
    'sendMessage returns text response',
    async () => {
  ...
  (SKIP ? it.skip : it)(
    'sendMessage with tool_context includes tools in request',
    async () => {
      const tokens: string[] = [];
      const result = await adapter.sendMessage({
        config: { api_key: API_KEY!, model: 'claude-haiku-4-5-20251001' },
