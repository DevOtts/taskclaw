...
const MODEL_PRICING: Record<string, { input: number; output: number }> = {
  ...
  // Meta via OpenRouter
  'meta-llama/llama-3.1-70b-instruct': { input: 0.00000052, output: 0.00000075 },
...
export class LangfuseService implements OnModuleDestroy {
  ...
  async getUsageSummary(
    ...
    // Aggregate by day
    const dayMap = new Map<string, { messages: number; tokens: number; cost: number }>();
    let totalTokens = 0;
    let totalCost = 0;

    for (const msg of messages) {
      const date = new Date(msg.created_at).toISOString().split('T')[0];
      const tokens = msg.metadata?.tokens_used || msg.metadata?.totalTokens || 0;
  ...
  async getUsageBreakdown(
    ...
    return {
      byTask: byTask.map(t => ({ ...t, cost: Math.round(t.cost * 1000000) / 1000000 })),
      byCategory: byCategory.map(c => ({ ...c, cost: Math.round(c.cost * 1000000) / 1000000 })),
    };
  }
