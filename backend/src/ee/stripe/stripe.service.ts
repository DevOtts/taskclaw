...
export class StripeService {
     ...
     */
    async handleWebhook(payload: Buffer, sig: string): Promise<{ received: boolean }> {
        const stripe = this.ensureStripe();
    ...
    private async handleCheckoutCompleted(session: Stripe.Checkout.Session) {
        ...
        if (!accountId || !planId) {
            this.logger.error('checkout.session.completed missing metadata (account_id or plan_id)');
            return;
        }
    ...
    private async handleSubscriptionDeleted(subscription: Stripe.Subscription) {
        ...
        if (error) {
            this.logger.error(`Failed to update canceled subscription: ${error.message}`);
        } else {
    ...
    async createProductAndPrice(plan: {
        ...
        if (!this.stripe) {
            this.logger.warn('Stripe not configured – skipping product/price creation');
            return null;
        }
    ...
    async updateProductAndPrice(
        ...
        if (priceChanged || intervalChanged) {
            ...
            this.logger.log(`Created new Stripe Price ${newPrice.id} for Product ${existingProductId}`);
     ...
     */
    async archiveProduct(productId: string, priceId: string | null): Promise<void> {
        ...
        try {
            ...
            this.logger.error(`Failed to archive Stripe product ${productId}: ${err.message}`);
        }
    }
}
