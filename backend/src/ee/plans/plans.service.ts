...
export class PlansService {
        ...
        if (this.stripeService.isConfigured()) {
            try {
                ...
                if (result) {
                    ...
                    if (patchError) {
                        this.logger.error(`Plan ${plan.id} created in Stripe but failed to save IDs: ${patchError.message}`);
                        stripe_sync_status = 'failed';
                        stripe_sync_error = 'Stripe product created but failed to save IDs to database';
                    } else {
                ...
                this.logger.error(`Failed to create Stripe product for plan ${plan.id}: ${err.message}`);
                stripe_sync_status = 'failed';
    ...
    async updatePlan(id: string, planData: any) {
        ...
        if (fetchError || !existingPlan) {
            throw new InternalServerErrorException(fetchError?.message || 'Plan not found');
        }
            ...
            try {
                if (existingPlan.stripe_product_id) {
                    ...
                    if (result && result.priceId !== existingPlan.stripe_price_id) {
                        ...
                        if (patchError) {
                            this.logger.error(`Plan ${id}: new Stripe price created but failed to save ID: ${patchError.message}`);
                            stripe_sync_status = 'failed';
                            stripe_sync_error = 'New Stripe price created but failed to save ID';
                        } else {
                            plan.stripe_price_id = result.priceId;
                            stripe_sync_status = 'synced';
                        }
                ...
                this.logger.error(`Failed to sync plan ${id} to Stripe: ${err.message}`);
                stripe_sync_status = 'failed';
                stripe_sync_error = err.message;
            }
        }
    ...
    async deletePlan(id: string) {
        ...
        if (plan?.stripe_product_id) {
            this.stripeService.archiveProduct(plan.stripe_product_id, plan.stripe_price_id)
                .catch(err => this.logger.error(`Failed to archive Stripe product: ${err.message}`));
        }
