...
function makeSupabaseAdmin(planName: string | null, resourceCount: number, countError?: any) {
  const subQuery = {
  ...
  const countQuery = {
    ...
    then: (resolve: any) =>
      Promise.resolve({ count: resourceCount, error: countError ?? null }).then(resolve),
  };
...
describe('PlanLimitGuard', () => {
  ...
  describe('community edition (non-cloud)', () => {
    it('skips all checks and returns true when EDITION !== cloud', async () => {
      ...
      const guard = new PlanLimitGuard(makeReflector('sources'), makeSupabaseAdmin('Hobby', 10) as any);
  ...
  describe('cloud edition — no @PlanResource decorator', () => {
    it('returns true when handler has no PlanResource decorator', async () => {
      ...
      const guard = new PlanLimitGuard(makeReflector(undefined), makeSupabaseAdmin('Hobby', 0) as any);
      const context = buildContext({ accountId: 'account-1' });
      await expect(guard.canActivate(context)).resolves.toBe(true);
    });
  });
  ...
  describe('cloud edition — no accountId in params', () => {
    it('returns true when accountId is missing from request params', async () => {
      process.env.EDITION = 'cloud';
      const guard = new PlanLimitGuard(makeReflector('sources'), makeSupabaseAdmin('Hobby', 0) as any);
  ...
  describe('Hobby plan limits', () => {
    beforeEach(() => { process.env.EDITION = 'cloud'; });

    it('blocks creation when sources count is at the Hobby limit (2)', async () => {
      const guard = new PlanLimitGuard(makeReflector('sources'), makeSupabaseAdmin('Hobby', 2) as any);
      const context = buildContext({ accountId: 'account-1' });
      await expect(guard.canActivate(context)).rejects.toThrow(ForbiddenException);
    });

    it('allows creation when sources count is below the Hobby limit', async () => {
      const guard = new PlanLimitGuard(makeReflector('sources'), makeSupabaseAdmin('Hobby', 1) as any);
      const context = buildContext({ accountId: 'account-1' });
      await expect(guard.canActivate(context)).resolves.toBe(true);
    });

    it('blocks when tasks count is at the Hobby limit (500)', async () => {
      const guard = new PlanLimitGuard(makeReflector('tasks'), makeSupabaseAdmin('Hobby', 500) as any);
      const context = buildContext({ accountId: 'account-1' });
      await expect(guard.canActivate(context)).rejects.toThrow(ForbiddenException);
    });
  ...
  describe('Pro plan', () => {
    ...
    it('allows unlimited conversations on Pro plan (-1)', async () => {
      const guard = new PlanLimitGuard(makeReflector('conversations'), makeSupabaseAdmin('Pro', 9999) as any);
      const context = buildContext({ accountId: 'account-1' });
      await expect(guard.canActivate(context)).resolves.toBe(true);
    });

    it('blocks sources when at Pro limit (10)', async () => {
      const guard = new PlanLimitGuard(makeReflector('sources'), makeSupabaseAdmin('Pro', 10) as any);
      const context = buildContext({ accountId: 'account-1' });
      await expect(guard.canActivate(context)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('Enterprise plan', () => {
    ...
    it('never blocks for any resource (all -1 unlimited)', async () => {
      for (const resource of ['sources', 'categories', 'tasks', 'skills']) {
        const guard = new PlanLimitGuard(makeReflector(resource), makeSupabaseAdmin('Enterprise', 99999) as any);
        const context = buildContext({ accountId: 'account-1' });
        await expect(guard.canActivate(context)).resolves.toBe(true);
      }
    });
  ...
  describe('no active subscription', () => {
    ...
    it('defaults to Hobby plan when no subscription found', async () => {
      const guard = new PlanLimitGuard(makeReflector('sources'), makeSupabaseAdmin(null, 2) as any);
      const context = buildContext({ accountId: 'account-1' });
  ...
  describe('count query error', () => {
    ...
    it('returns true (fail-open) when the resource count query fails', async () => {
      const supabaseAdmin = makeSupabaseAdmin('Hobby', 0, { message: 'DB error' });
      const guard = new PlanLimitGuard(makeReflector('sources'), supabaseAdmin as any);
      const context = buildContext({ accountId: 'account-1' });
      await expect(guard.canActivate(context)).resolves.toBe(true);
    });
  });
